...
describe('BotSettings', () => {
  ...
  it('disables inputs when bot is disabled', () => {
    const disabledSettingsStore = {
      ...mockSettingsStore,
      desktopSettings: { ...mockSettingsStore.desktopSettings }
    }
    // Remove the problematic botEnabled property since it's not in standard DesktopSettings
    
    render(<BotSettings settingsStore={disabledSettingsStore as any} />)
