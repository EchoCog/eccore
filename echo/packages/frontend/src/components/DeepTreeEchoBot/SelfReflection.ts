...
export class SelfReflection {
  ...
  private async processReflectionResults(reflectionResult: string): Promise<void> {
    try {
      ...
      const personalityMatch = reflectionResult.match(/Revised Personality:([\s\S]*?)(?=Suggested Adjustments|$)/)
      ...
      const preferencesMatch = reflectionResult.match(/Suggested Adjustments to Preferences:([\s\S]*?)(?=Suggested Adjustments to Cognitive|$)/)
      ...
      const cognitiveMatch = reflectionResult.match(/Suggested Adjustments to Cognitive Parameters:([\s\S]*?)(?=Overall|$)/)
  ...
  private extractReflectionSummary(reflectionResult: string): string {
    ...
    const summaryMatch = reflectionResult.match(/(?:Insights|Summary|Conclusion):([\s\S]*?)(?=\[|$)/)
