...
export async function initDeepTreeEchoBot(): Promise<void> {
  try {
    ...
    botInstance = new DeepTreeEchoBot({
      ...
      useParallelProcessing: true, // Default to enabled for multi-cognitive processing
      cognitiveKeys
    })
...
function registerMessageHandlers(): void {
  ...
  // Note: We need to get the current account ID, but since this is a global listener,
  // we'll handle all accounts. The actual account filtering will be done in handleNewMessage
  const currentAccountId = (window as any).__selectedAccountId || 1 // Default to account 1 if none selected
  onDCEvent(currentAccountId, 'IncomingMsg', (event: any) => {
    // Extract chatId, msgId from the event
    const { chatId, msgId } = event
    handleNewMessage(currentAccountId, chatId, msgId)
  })
