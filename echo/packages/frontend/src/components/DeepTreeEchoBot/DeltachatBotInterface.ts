    ...
    try {
      if (!this.botAccountId) {
        ...
        if (accounts.length > 0) {
          ...
          const chatId = await BackendRemote.rpc.createGroupChat(accounts[0].id, name, false)
          
          // Add members
          for (const address of memberAddresses) {
            try {
              // Look up or create contact
              const contactId = await BackendRemote.rpc.createContact(accounts[0].id, address, address)
              // Add contact to chat
                              await BackendRemote.rpc.addContactToChat(accounts[0].id, chatId, contactId)
            } catch (error) {
        ...
        // Create group chat with specified name and type 0 (normal group)
        const chatId = await BackendRemote.rpc.createGroupChat(this.botAccountId, name, false)
