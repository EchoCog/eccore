...
export class DeepTreeEchoBot {
  ...
  constructor(options: DeepTreeEchoBotOptions) {
    ...
    // Initialize all components using singleton instances
    ...
    this.visionCapabilities = VisionCapabilities.getInstance()
    // Configure vision capabilities
    this.visionCapabilities.updateOptions({
      enabled: options.visionEnabled
    })
    
    this.webAutomation = PlaywrightAutomation.getInstance()
    // Configure web automation
  ...
  async processMessage(
    ...
    try {
      ...
      let memories: Memory[] = []
      if (this.options.memoryEnabled) {
        memories = this.memoryStore.getMemoriesByChat(chatId).slice(-10)
      }
      ...
      const llmResponse = await this.llmService.generateResponse(
        userMessage,
        [systemPrompt, memories.map(m => `${m.sender}: ${m.text}`).join('\n')]
      )
      ...
      if (this.options.memoryEnabled) {
        await this.memoryStore.storeMemory({
          ...
          sender: 'bot',
          chatId,
          messageId: 0 // Use 0 for generated responses
        })
      }
  ...
  private registerCommandHandlers(): void {
    ...
    this.commandHandlers.set('/memory', async (args, messageData) => {
      ...
      switch (subcommand) {
        case 'status':
          const memories = this.memoryStore.getMemoriesByChat(messageData.chatId)
          const stats = {
